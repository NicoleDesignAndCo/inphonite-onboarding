...
function PendingDashboard({ state, patch, go }) {
  ...
  // Demo affordance — flip the whole account between "in setup" and
  // "completed & verified" so clients can preview the finished state.
  const setCompleted = (on) => {
    if (on) {
      patch({
        phase: "ready", navUnlocked: true,
        setup: { business: "done", channels: "done", tollfree: "done", ehr: "done", review: "done" },
        tfn: { ...state.tfn, state: "ready" },
      });
    } else {
      patch({
        phase: "collecting", navUnlocked: false,
  ...
  return (
    <div className="col" style={{ gap: 24, maxWidth: 1240, margin: "0 auto" }}>
      ...
      <div className="dash-hero slide-in">
        {/* Demo toggle — preview the completed & verified account */}
        <div className="hero-demo-toggle">
          <span className="hdt-label">Demo: account completed &amp; verified</span>
          <button type="button" role="switch" aria-checked={allDone}
                  className={"hdt-switch" + (allDone ? " on" : "")}
                  onClick={() => setCompleted(!allDone)}>
            <span className="hdt-knob" />
          </button>
        </div>

        <div className="eyebrow">
          {allDone
            ? <><Icon name="check_circle" size={14} /> Account Active · Verified</>
            : <><Icon name="hourglass_top" size={14} /> Setup In Progress</>}
        </div>
        <h1>Welcome, Eleanor</h1>
        {!allDone && (
          <>
            <p>Your account isn't verified yet. Finish the steps below to activate {state.practice.name}.</p>
